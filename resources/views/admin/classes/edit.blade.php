...
<div class="max-w-2xl mx-auto bg-white rounded-xl shadow p-8">
    ...
    <h3 class="text-lg font-semibold text-gray-700 mb-6">Edit Class — {{ $class->name }}</h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="level"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        required>
                    <option value="Form 1"  {{ $class->level == 'Form 1'  ? 'selected' : '' }}>Form 1</option>
                    <option value="Form 2"  {{ $class->level == 'Form 2'  ? 'selected' : '' }}>Form 2</option>
                    <option value="Form 3"  {{ $class->level == 'Form 3'  ? 'selected' : '' }}>Form 3</option>
                    <option value="Form 4"  {{ $class->level == 'Form 4'  ? 'selected' : '' }}>Form 4</option>
                    <option value="Grade 7" {{ $class->level == 'Grade 7' ? 'selected' : '' }}>Grade 7</option>
                    <option value="Grade 8" {{ $class->level == 'Grade 8' ? 'selected' : '' }}>Grade 8</option>
                    <option value="Grade 9" {{ $class->level == 'Grade 9' ? 'selected' : '' }}>Grade 9</option>
                </select>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Stream</label>
                <input type="text" name="stream" value="{{ old('stream', $class->stream) }}"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
